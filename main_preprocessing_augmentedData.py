...
import os
import h5py
from utils.preprocessing_augmentation import convert2mel, normalize_amplitude, windowing
...
parser.add_argument('-d','--data_augmentation',dest='data_augmentation',action='store',required=False, type=str,default='False')

args = parser.parse_args()
type_training= args.type_training
path_to_metadata =  args.directory +'FSDnoisy18k.meta/' + type_training + '_set.csv'
hdf5_name = "processed_data_" + type_training  + args.data_augmentation  +   ".hdf5" 
if args.data_augmentation:
    audio_path= args.audio_dir + "data_" + type_training +"_" +args.data_augmentation
else:
...
data_processed = [convert2mel(audio,audio_path,fs, n_fft,fmax,n_mels,hop_length_samples, windows_size_f,type_training,args.data_augmentation) for ii,audio in enumerate(fname)]
